...
### Fixed
- **YouTube 403 Forbidden**: Bypassed YouTube strict security checks by spoofing smart TV and iOS API clients via `yt-dlp` `extractor_args` for flawless 1080p/4K formats downloading.
- **Auto-Updater**: Added background engine upgrade command on startup to proactively fetch upstream fixes and ensure consistent backend stability.
